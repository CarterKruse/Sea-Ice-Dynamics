r'''
Euler 2D Fields - Create Dataset
==================
...
def save_initial_conditions(density_field, pressure_field, file_name):
    '''
    Saves the initial density and pressure fields to a text file.
    '''    
    with open(file_name, 'w') as file:
        ...
        for i in range(density_field.shape[0]):
            ...
            for j in range(density_field.shape[1]):
                row.append(f'{density_field[i, j]}')
            file.write(f"[{', '.join(row)}],\n")
        file.write(']\n\n')

        # pressure field
        file.write('[\n')
        for i in range(pressure_field.shape[0]):
            row = []
            for j in range(pressure_field.shape[1]):
                row.append(f'{pressure_field[i, j]}')
            file.write(f"[{', '.join(row)}],\n")
        file.write(']\n\n')

def save_velocity_vectors(solution, file_name):
    '''
    Saves the velocity vectors (for each time step) to a text file.
    '''
    x_velo = x_velocity(solution)
    y_velo = y_velocity(solution)

    with open(file_name, 'a') as file:
        # velocity vectors
        file.write('[\n')
        for i in range(x_velo.shape[0]):
            row = []
            for j in range(x_velo.shape[1]):
                # format each vector as a tuple
                row.append(f"({x_velo[i, j]}, {y_velo[i, j]})")
            file.write(f"[{', '.join(row)}],\n")
        file.write(']\n\n')
...
def run_simulation_and_save(claw, i, use_petsc = False):
    ...
    # reset file
    with open('./_data/velo' + str(i) + '.txt', 'w') as file:
        file.write('')
    ...
    for frame in range(claw.num_output_times + 1):
        ...
        save_velocity_vectors(sol, './_data/velo' + str(i) + '.txt')

if __name__ == '__main__':
    import logging
    logger = logging.getLogger('pyclaw')
    logger.setLevel(logging.WARNING) # CRITICAL, ERROR, WARNING, INFO, DEBUG
